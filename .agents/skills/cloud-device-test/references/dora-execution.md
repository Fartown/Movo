...
3. 续期遵守用户时长和平台上限；按当前 Dora 指南安排必要的续租守护，记录 PID、日志、到期时间和归属。设备默认保留，测试结束后继续维持租约。只有主 agent 列明设备并取得用户明确释放确认后，才停止本任务守护、释放对应平台租约并读回；不得因任务结束、切换设备或用户未回复自行释放或停止续租。平台限制导致无法续期时及时报告实际到期时间与限制。
