...
int main(int argc, char **argv) {
    if (argc < 5 || strcmp(argv[3], "--")) {
        fprintf(stderr, "MOVO_PTY_INVALID_ARGUMENT\n");
        return 64;
    }
    int rows = dimension(argv[1]);
    int columns = dimension(argv[2]);
    if (rows < 0 || columns < 0) {
        fprintf(stderr, "MOVO_PTY_INVALID_SIZE\n");
        return 64;
    }
    ...
    if (master < 0 || grantpt(master) < 0 || unlockpt(master) < 0) {
        fprintf(stderr, "MOVO_PTY_OPEN_FAILED\n");
    ...
    if (!child) {
        ...
        fprintf(stderr, "MOVO_PTY_EXEC_FAILED errno=%d\n", errno);
        _exit(127);
    }
