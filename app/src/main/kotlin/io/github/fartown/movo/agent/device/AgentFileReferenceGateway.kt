...
import io.github.fartown.movo.agent.model.hasUnsupportedControlCharacter
    ...
    internal companion object {
        ...
        private fun validationCommand(path: String): String {
            ...
            return buildString {
                ...
                append("movo_path=\$(readlink -f ").append(quotedPath).append(" 2>/dev/null) || exit ")
                append(EXIT_PATH_NOT_FOUND).append("; ")
                append("[ -n \"\$movo_path\" ] || exit ").append(EXIT_PATH_NOT_FOUND).append("; ")
                append("if [ -f \"\$movo_path\" ]; then movo_kind=").append(KIND_FILE).append("; ")
                append("elif [ -d \"\$movo_path\" ]; then movo_kind=").append(KIND_DIRECTORY).append("; ")
                append("else exit ").append(EXIT_UNSUPPORTED_TYPE).append("; fi; ")
                append("printf '%s\\n%s' \"\$movo_kind\" \"\$movo_path\"")
            }
        }
