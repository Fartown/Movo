...
internal class RootShellDeviceController(
    ...
    fun clipboardSet(context: Context, text: String): String {
        ...
        val ok = serviceResult?.ok ?: runCatching {
            ...
            clipboard.setPrimaryClip(ClipData.newPlainText("movo_agent", text))
            true
        }.getOrDefault(false)
        val json = JSONObject()
            .put("ok", ok)
    ...
    private fun dumpUiNodes(maxNodes: Int): List<UiNode> {
        ...
        val result = runSuText(
            "uiautomator dump --compressed /data/local/tmp/movo_window.xml >/dev/null && " +
                "cat /data/local/tmp/movo_window.xml && rm -f /data/local/tmp/movo_window.xml",
            timeoutSeconds = 10
        )
