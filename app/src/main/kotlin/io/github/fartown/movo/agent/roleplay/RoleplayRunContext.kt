...
internal data class RoleplayRunContext(
    ...
    private fun isDialogue(message: JSONObject): Boolean =
        message.optString("role") in setOf("user", "assistant") &&
            !message.optBoolean("_movo_observation") && !message.optBoolean("_movo_context_summary") &&
    ...
    private fun personaPrompt(before: String, after: String): String = buildString {
        appendLine("本会话的角色人格：${card.name}。以该人物的身份和语气交流，不要在普通剧情中自称 Movo。")
    ...
    companion object {
        private const val PERSONA_MARKER = "_movo_character_profile"
        ...
        suspend fun resolve(
            ...
            val raw = MovoDatabase.get(context).conversationDao().roleplayJson(conversationId)
