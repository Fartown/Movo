...
internal class AlpineEnvironmentInstaller(
    ...
    private suspend fun installRootfs(
        ...
        val command = """
            ...
            "${'$'}movo_busybox" tar -xzf "${'$'}movo_archive" -C "${'$'}movo_temporary" || exit 67
            ...
            printf '%s\n' \
              ...
              ${shellQuote("${APK_MIRROR_BASE_URLS.first()}/v3.24/community")} > "${'$'}movo_temporary/etc/apk/repositories"
            "${'$'}movo_busybox" mkdir -p "${'$'}movo_temporary/usr/local/bin"
            printf '%s\n' '#!/bin/sh' > "${'$'}movo_temporary/usr/local/bin/movo-apk"
            printf %s ${shellQuote(apkMirrorScriptBody())} >> "${'$'}movo_temporary/usr/local/bin/movo-apk"
            "${'$'}movo_busybox" chmod 0755 "${'$'}movo_temporary/usr/local/bin/movo-apk"
            printf ${shellQuote(markerBody)} > "${'$'}movo_temporary/${AlpineEnvironmentPaths.READY_MARKER}"
            "${'$'}movo_busybox" chmod 0644 "${'$'}movo_temporary/${AlpineEnvironmentPaths.READY_MARKER}"
    ...
    private suspend fun installCommonTools(rootfs: File): Boolean {
        ...
        val command = """
            ...
            printf '%s\n' '#!/bin/sh' > /usr/local/bin/movo-apk
            printf %s ${shellQuote(apkMirrorScriptBody())} >> /usr/local/bin/movo-apk
            chmod 0755 /usr/local/bin/movo-apk
            /usr/local/bin/movo-apk install $packages || exit 70
            cat > /${AlpineEnvironmentPaths.COMMON_TOOLS_MARKER} <<'MOVO_TOOLSET_EOF'
    ...
    companion object {
        ...
        private fun apkMirrorScriptBody(): String = buildString {
            ...
            append("for movo_apk_mirror in ${APK_MIRROR_BASE_URLS.joinToString(" ")}; do ")
            append("printf '%s\\n' \"${'$'}movo_apk_mirror/v3.24/main\" \"${'$'}movo_apk_mirror/v3.24/community\" > /etc/apk/repositories || exit 65; ")
            append("if apk update && apk add --no-cache \"${'$'}@\"; then exit 0; fi; ")
            append("done; exit 1;; ")
            append("update) for movo_apk_mirror in ${APK_MIRROR_BASE_URLS.joinToString(" ")}; do ")
            append("printf '%s\\n' \"${'$'}movo_apk_mirror/v3.24/main\" \"${'$'}movo_apk_mirror/v3.24/community\" > /etc/apk/repositories || exit 65; ")
            append("apk update && exit 0; done; exit 1;; ")
            append("*) echo \"usage: movo-apk install PACKAGE... | update\" >&2; exit 64;; esac")
        }
...
internal object InstallerShellRunner {
    ...
    suspend fun run(
        ...
        val reader = thread(name = "movo-alpine-installer-output", isDaemon = true) {
            runCatching {
