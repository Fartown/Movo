...
/** Movo 管理的 Linux 工具环境路径；内部历史包名不参与对外展示。 */
internal object AlpineEnvironmentPaths {
    const val READY_MARKER = LinuxEnvironmentPaths.READY_MARKER
    const val COMMON_TOOLS_MARKER = ".movo-common-tools-ready"
    const val APK_ANALYSIS_MARKER = ".movo-apk-analysis-ready"
    const val PYTHON_TOOLS_MARKER = ".movo-python-tools-ready"
    const val NODE_TOOLS_MARKER = ".movo-node-tools-ready"
    const val SSH_TOOLS_MARKER = ".movo-ssh-tools-ready"
    const val KIMI_TOOLS_MARKER = ".movo-kimi-tools-ready"
