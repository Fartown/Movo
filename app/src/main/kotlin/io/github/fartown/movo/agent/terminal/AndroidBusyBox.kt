...
internal object AndroidBusyBox {
    ...
    fun discoveryScript(variable: String = "movo_busybox"): String {
        require(variable.matches(Regex("[a-z_][a-z0-9_]*"))) { "非法 Shell 变量名" }
        val quotedCandidates = candidates.joinToString(" ") { shellQuote(it) }
        return "$variable=''; " +
            "for movo_candidate in $quotedCandidates; do " +
            "if [ -x \"${'$'}movo_candidate\" ]; then $variable=\"${'$'}movo_candidate\"; break; fi; " +
            "done; " +
