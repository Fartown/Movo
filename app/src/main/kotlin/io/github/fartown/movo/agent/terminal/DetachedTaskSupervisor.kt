...
internal class DetachedTaskSupervisor(
    ...
    companion object {
        const val DEFAULT_DAEMON_DIR = "/data/local/tmp/movo/daemon"
    ...
    fun start(
        ...
        val innerScript = "echo \$\$ > ${shellQuote(wirePidFile)}; " +
            "export $MOVO_PROCESS_OWNER_ENV=${shellQuote(token)}; " +
    ...
    fun list(): List<DetachedTaskStatus> {
        ...
        tasks.groupBy { it.identity }.forEach { (identity, group) ->
            ...
            val probeScript = aliveCheckFunction() + "\n" + group.joinToString("\n") { task ->
                "if movo_alive ${task.pid} ${shellQuote(ownerProof(task))}; " +
    ...
    private fun stopTaskProcess(task: DetachedTask): Boolean {
        val stopScript = buildString {
            ...
            appendLine("if movo_alive ${task.pid} ${shellQuote(ownerProof(task))}; then")
            ...
            appendLine("  fi")
            appendLine("  sleep 0.1")
            appendLine("  movo_alive ${task.pid} ${shellQuote(ownerProof(task))} && exit 81")
            appendLine("fi")
    ...
    private fun ownerProof(task: DetachedTask): String = "$MOVO_PROCESS_OWNER_ENV=${task.token}"
    ...
    private fun startUserDaemon(id: String, token: String, command: String, cwd: String, environment: TerminalEnvironment): DaemonStartResult {
        ...
        return synchronized(launchLock) {
            if (!acquireUserLease(lease) {
                ...
                }) return@synchronized DaemonStartResult.Failed("BACKGROUND_START_NOT_ALLOWED", "请返回 Movo 后重新启动后台任务")
            if (stopped.get()) {
            ...
            val process = try {
                ProcessBuilder("sh", "-c", launcher).apply {
                    environment()[MOVO_PROCESS_OWNER_ENV] = token
            ...
            thread(name = "movo-daemon-wait", isDaemon = true) {
    ...
    private fun adoptUserTask(task: DetachedTask): Boolean {
        ...
        thread(name = "movo-daemon-adopt", isDaemon = true) {
            try {
                while (!stopped.get()) {
                    val result = runOneShotShell(
                        processSupervisor = oneShotSupervisor, identity = "user",
                        command = aliveCheckFunction() + "\nif movo_alive ${task.pid} ${shellQuote(ownerProof(task))}; then echo alive; else echo exited; fi",
                        timeoutSeconds = 5,
                    )
