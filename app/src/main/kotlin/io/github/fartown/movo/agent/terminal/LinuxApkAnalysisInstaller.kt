...
internal fun linuxApkJavaInstallCommand(distribution: LinuxDistribution): String =
    when (distribution) {
        LinuxDistribution.ALPINE -> "/usr/local/bin/movo-apk install openjdk25-jdk"
        LinuxDistribution.DEBIAN -> "/usr/local/bin/movo-apt install openjdk-25-jdk-headless"
    }
...
internal class LinuxApkAnalysisInstaller(
    ...
    private suspend fun activateStaging(rootfs: File, staging: File): Boolean {
        ...
        val profileRoot = File(rootfs, "opt/movo/apk-analysis")
        ...
        val command = """
            ...
            "${'$'}movo_busybox" mkdir -p ${shellQuote(profileRoot.absolutePath)} || exit 70
            "${'$'}movo_busybox" rm -rf ${shellQuote(installing.absolutePath)} ${shellQuote(previous.absolutePath)}
            "${'$'}movo_busybox" mv ${shellQuote(staging.absolutePath)} ${shellQuote(installing.absolutePath)} || exit 71
            "${'$'}movo_busybox" chmod 0755 \
            ...
            movo_link_commands() {
              "${'$'}movo_busybox" mkdir -p ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)} || return 1
              for movo_command in java jadx apktool smali baksmali; do
                "${'$'}movo_busybox" rm -f ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)}/"${'$'}movo_command"
              done
              "${'$'}movo_busybox" ln -s ../../../opt/movo/apk-analysis/current/bin/java \
                ${shellQuote(File(rootfs, "usr/local/bin/java").absolutePath)} || return 1
              "${'$'}movo_busybox" ln -s ../../../opt/movo/apk-analysis/current/jadx/bin/jadx \
                ${shellQuote(File(rootfs, "usr/local/bin/jadx").absolutePath)} || return 1
              for movo_command in apktool smali baksmali; do
                "${'$'}movo_busybox" ln -s ../../../opt/movo/apk-analysis/current/bin/"${'$'}movo_command" \
                  ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)}/"${'$'}movo_command" || return 1
              done
            }
            movo_restore_previous() {
              "${'$'}movo_busybox" rm -rf ${shellQuote(current.absolutePath)}
              if [ -d ${shellQuote(previous.absolutePath)} ]; then
                "${'$'}movo_busybox" mv ${shellQuote(previous.absolutePath)} ${shellQuote(current.absolutePath)}
                movo_link_commands || true
              fi
            }
            if [ -d ${shellQuote(current.absolutePath)} ]; then
              "${'$'}movo_busybox" mv ${shellQuote(current.absolutePath)} ${shellQuote(previous.absolutePath)} || exit 73
            fi
            "${'$'}movo_busybox" mv ${shellQuote(installing.absolutePath)} ${shellQuote(current.absolutePath)} || {
              movo_restore_previous
              exit 74
            }
            movo_link_commands || {
            ...
            "${'$'}movo_busybox" rm -f ${shellQuote(File(rootfs, AlpineEnvironmentPaths.APK_ANALYSIS_MARKER).absolutePath)}
        """.trimIndent()
        val result = InstallerShellRunner.run(
            command = command,
            timeoutSeconds = 60,
    ...
    private suspend fun verifyAndMark(rootfs: File): Boolean {
        val command = """
            ...
            cat > /${AlpineEnvironmentPaths.APK_ANALYSIS_MARKER} <<'MOVO_APK_ANALYSIS_EOF'
            profile=${AlpineEnvironmentPaths.APK_ANALYSIS_REVISION}
            jadx=$JADX_VERSION
            apktool=$APKTOOL_VERSION
            smali=$SMALI_VERSION
            MOVO_APK_ANALYSIS_EOF
    ...
    private suspend fun cleanupAfterSuccess(rootfs: File, artifacts: Collection<File>) {
        ...
        val previous = File(rootfs, "opt/movo/apk-analysis/previous")
        if (LinuxEnvironmentPaths.backendOf(rootfs.absolutePath) == LinuxExecutionBackend.PROOT) {
        ...
        val command = """
            ${AndroidBusyBox.discoveryScript()}
            [ -n "${'$'}movo_busybox" ] || exit 127
            "${'$'}movo_busybox" rm -rf ${shellQuote(previous.absolutePath)}
        """.trimIndent()
        InstallerShellRunner.run(command, 30, TerminalEnvironment.ANDROID)
    }

    private fun activateRootless(rootfs: File, staging: File): Boolean {
        val profileRoot = File(rootfs, "opt/movo/apk-analysis").apply { mkdirs() }
        val current = File(profileRoot, "current")
        val previous = File(profileRoot, "previous")
        ...
        try {
            ...
            listOf("java", "jadx", "apktool", "smali", "baksmali").forEach { name ->
                ...
                java.nio.file.Files.createSymbolicLink(path, java.nio.file.Path.of("../../../opt/movo/apk-analysis/current/$relative"))
            }
            File(rootfs, AlpineEnvironmentPaths.APK_ANALYSIS_MARKER).delete()
            return true
    ...
    companion object {
        ...
        internal val APKTOOL_WRAPPER = """
            ...
            case "${'$'}{1:-}" in
              b|build)
                echo "APKTOOL_BUILD_UNAVAILABLE: Movo APK 分析档案暂不包含 ARM64 AAPT2，仅支持解码与检查。" >&2
                exit 64
                ;;
            esac
            exec java -jar /opt/movo/apk-analysis/current/lib/apktool.jar "${'$'}@"
        """.trimIndent() + "\n"
        ...
        private fun javaJarWrapper(fileName: String): String =
            "#!/bin/sh\nexec java -jar /opt/movo/apk-analysis/current/lib/$fileName \"${'$'}@\"\n"
    }
}
