    ...
    val PYTHON = LinuxPackageProfile(
        ...
        specs = mapOf(
            LinuxDistribution.ALPINE to LinuxPackageSpec(
                ...
                setupScript = """
                    UV_PYTHON_INSTALL_DIR=/opt/movo/python UV_PYTHON_BIN_DIR=/usr/local/bin UV_PYTHON_INSTALL_BIN=1 uv python install --default --force
                """.trimIndent(),
            ),
...
internal class LinuxPackageProfileInstaller(
    ...
    private suspend fun installLocked(
        ...
        val packageHelper = when (distribution) {
            LinuxDistribution.ALPINE -> "/usr/local/bin/movo-apk"
            LinuxDistribution.DEBIAN -> "/usr/local/bin/movo-apt"
        }
        ...
        val activateCommand = buildString {
            ...
            append("cat > /").append(profile.markerName).append(" <<'MOVO_PROFILE_EOF'\n")
            append("profile=").append(profile.revision).append('\n')
            append("MOVO_PROFILE_EOF\n")
