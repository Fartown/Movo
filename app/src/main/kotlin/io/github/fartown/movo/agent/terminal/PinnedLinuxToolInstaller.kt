...
internal class PinnedLinuxToolInstaller(
    ...
    private suspend fun activateRootless(tool: ManagedLinuxTool, artifact: VerifiedArtifact, archive: File, rootfs: File): Boolean {
        ...
        val versions = File(rootfs, "opt/movo/$name")
        val target = File(versions, artifact.version)
        val staging = File(rootfs, "opt/movo/$name.installing")
        try {
            ...
            when (tool) {
                ...
                ManagedLinuxTool.NODE -> listOf("node", "npm", "npx").forEach { command ->
                    ...
                    java.nio.file.Files.createSymbolicLink(link, java.nio.file.Path.of("../../../opt/movo/node/${artifact.version}/bin/$command"))
                }
            }
            return true
        } finally {
    ...
    private fun uvActivationCommand(
        ...
        val target = File(rootfs, "opt/movo/uv/${artifact.version}")
        val staging = File(rootfs, "opt/movo/uv.installing")
        ...
        return """
            ...
            movo_staging=${shellQuote(staging.absolutePath)}
            movo_target=${shellQuote(target.absolutePath)}
            "${'$'}movo_busybox" rm -rf "${'$'}movo_staging"
            "${'$'}movo_busybox" mkdir -p "${'$'}movo_staging" || exit 66
            "${'$'}movo_busybox" tar -xzf "${'$'}movo_archive" -C "${'$'}movo_staging" --strip-components=1 || exit 67
            "${'$'}movo_busybox" mkdir -p ${shellQuote(versionsRoot.parentFile!!.absolutePath)} ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)} || exit 66
            "${'$'}movo_busybox" rm -rf ${shellQuote(versionsRoot.absolutePath)}
            "${'$'}movo_busybox" mkdir -p ${shellQuote(versionsRoot.absolutePath)} || exit 66
            "${'$'}movo_busybox" mv "${'$'}movo_staging" "${'$'}movo_target" || exit 69
            "${'$'}movo_busybox" chmod 0755 "${'$'}movo_target/uv" "${'$'}movo_target/uvx" || exit 70
            ...
            "${'$'}movo_busybox" chmod 0755 ${shellQuote(uvPath.absolutePath)} ${shellQuote(uvxPath.absolutePath)} || exit 71
        """.trimIndent()
    }

    private fun uvWrapper(version: String, command: String): String = """
        #!/bin/sh
        export UV_PYTHON_INSTALL_DIR=/opt/movo/python
        export UV_PYTHON_BIN_DIR=/usr/local/bin
        export UV_PYTHON_INSTALL_BIN=1
        export UV_TOOL_DIR=/opt/movo/uv-tools
        export UV_TOOL_BIN_DIR=/usr/local/bin
        exec /opt/movo/uv/$version/$command "${'$'}@"
    """.trimIndent() + "\n"
    ...
    private fun nodeActivationCommand(
        ...
        val target = File(rootfs, "opt/movo/node/${artifact.version}")
        val staging = File(rootfs, "opt/movo/node.installing")
        val versionsRoot = target.parentFile!!
        ...
        return """
            ...
            "${'$'}movo_busybox" rm -rf "${'$'}movo_staging"
            "${'$'}movo_busybox" mkdir -p "${'$'}movo_staging" || exit 66
            "${'$'}movo_busybox" tar -xJf "${'$'}movo_archive" -C "${'$'}movo_staging" --strip-components=1 || exit 67
            "${'$'}movo_busybox" mkdir -p ${shellQuote(versionsRoot.parentFile!!.absolutePath)} ${shellQuote(localBin.absolutePath)} || exit 66
            "${'$'}movo_busybox" rm -rf ${shellQuote(versionsRoot.absolutePath)}
            "${'$'}movo_busybox" mkdir -p ${shellQuote(versionsRoot.absolutePath)} || exit 66
            "${'$'}movo_busybox" mv "${'$'}movo_staging" "${'$'}movo_target" || exit 69
            "${'$'}movo_busybox" ln -sfn ${shellQuote("/opt/movo/node/${artifact.version}/bin/node")} ${shellQuote(File(localBin, "node").absolutePath)} || exit 71
            "${'$'}movo_busybox" ln -sfn ${shellQuote("/opt/movo/node/${artifact.version}/bin/npm")} ${shellQuote(File(localBin, "npm").absolutePath)} || exit 71
            "${'$'}movo_busybox" ln -sfn ${shellQuote("/opt/movo/node/${artifact.version}/bin/npx")} ${shellQuote(File(localBin, "npx").absolutePath)} || exit 71
        """.trimIndent()
    }
}
