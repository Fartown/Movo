...
internal object RootlessLinuxInstaller {
    ...
    suspend fun installBase(artifact: VerifiedArtifact, archive: File, rootfs: File, distribution: LinuxDistribution): Boolean {
        ...
        try {
            ...
            if (staging.exists() && !staging.deleteRecursively()) throw RootlessInstallFailure("STAGING_CLEANUP_FAILED", "无法清理未完成安装，请重启 Movo 后重试")
            ...
            val helper = when (distribution) {
                LinuxDistribution.ALPINE -> {
                    ...
                    "movo-apk" to AlpineEnvironmentInstaller.apkMirrorScript()
                }
                LinuxDistribution.DEBIAN -> {
                    ...
                    File(staging, "etc/apt/apt.conf.d/99movo-rootless").writeText("APT::Sandbox::User \"root\";\nAcquire::Retries \"2\";\n")
                    File(staging, "usr/sbin/policy-rc.d").apply { writeText("#!/bin/sh\nexit 101\n"); setExecutable(true, false) }
                    "movo-apt" to DebianEnvironmentInstaller.aptMirrorScript()
                }
            }
