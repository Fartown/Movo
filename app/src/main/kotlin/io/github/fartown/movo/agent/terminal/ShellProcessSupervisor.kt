...
internal class ShellProcessSupervisor(
    ...
    fun startShellProcess(
        ...
        val ownershipFile = runCatching {
            File.createTempFile("movo-terminal-", ".owner")
    ...
    internal fun buildTrackedShellLauncher(
        ...
        val exportOwner = "export $MOVO_PROCESS_OWNER_ENV=${shellQuote(ownershipToken)}"
        val cleanupGroup =
                "movo_cleanup_proc_group() { " +
                ...
                "done; }; " +
                "movo_cleanup_ps_group() { " +
                "for pid in ${'$'}(ps -axo pid=,pgid= | " +
                "awk -v group=\"${'$'}${'$'}\" '${'$'}2 == group && ${'$'}1 != group { print ${'$'}1 }'); do " +
                "kill -9 \"${'$'}pid\" 2>/dev/null; done; }; " +
                "if [ -d /proc/${'$'}${'$'} ]; then " +
                "movo_cleanup_proc_group; movo_cleanup_proc_group; " +
                "else movo_cleanup_ps_group; movo_cleanup_ps_group; fi"
        val groupScript =
            "printf '%s group\\n' \"${'$'}${'$'}\" > $path; " +
                "$payload; movo_status=${'$'}?; $cleanupGroup; exit ${'$'}movo_status"
        val treeScript =
            "printf '%s tree\\n' \"${'$'}${'$'}\" > $path; $payload"
    ...
    private fun buildPtyLauncher(
        ...
        val run = "\"\$movo_busybox\" script -qfc ${shellQuote(ptyScript)} /dev/null"
        return "$exportOwner; $discovery; [ -n \"\$movo_busybox\" ] || { $unavailable; }; " +
    ...
    internal fun buildAndroidPayload(identity: String, command: String?): String {
        ...
        val discovery = AndroidBusyBox.discoveryScript()
        return "$discovery; " +
            "if [ -n \"${'$'}movo_busybox\" ]; then " +
            "export MOVO_BUSYBOX=\"${'$'}movo_busybox\" ASH_STANDALONE=1; " +
            "\"${'$'}movo_busybox\" ash $shellArgument; " +
    ...
    internal fun buildLinuxPayload(
        ...
        val mountsBlock = sharedMounts.joinToString("\n") { mount ->
            "movo_mount_optional ${shellQuote(mount.sourcePath)} " +
                "\"\$movo_rootfs${SharedFolderMounts.LINUX_MOUNTS_ROOT}/${mount.name}\" bind"
        }
        val innerScriptHead = """
            movo_rootfs=${'$'}1
            movo_busybox=${'$'}2
            ...
            movo_mount_required() {
              ...
              movo_target=${'$'}2
              movo_options=${'$'}3
              "${'$'}movo_busybox" mkdir -p "${'$'}movo_target" || exit 125
              "${'$'}movo_busybox" mount -o "${'$'}movo_options" "${'$'}movo_source" "${'$'}movo_target" || exit 125
            }
            movo_mount_optional() {
              movo_source=${'$'}1
              movo_target=${'$'}2
              movo_options=${'$'}3
              "${'$'}movo_busybox" mkdir -p "${'$'}movo_target" 2>/dev/null || return 0
              "${'$'}movo_busybox" mount -o "${'$'}movo_options" "${'$'}movo_source" "${'$'}movo_target" 2>/dev/null || true
            }
            "${'$'}movo_busybox" mount -t proc proc "${'$'}movo_rootfs/proc" || exit 125
            movo_mount_required /dev "${'$'}movo_rootfs/dev" rbind
            movo_mount_optional /sys "${'$'}movo_rootfs/sys" rbind
            if [ -d /storage/emulated/0 ]; then
              movo_mount_optional /storage/emulated/0 "${'$'}movo_rootfs/storage/emulated/0" bind
            fi
            [ -d /data/local/tmp ] || exit 125
            movo_mount_required /data/local/tmp "${'$'}movo_rootfs/data/local/tmp" bind
            "${'$'}movo_busybox" mkdir -p /data/local/tmp/movo || exit 125
            movo_mount_required /data/local/tmp/movo "${'$'}movo_rootfs/workspace" bind
        """.trimIndent()
        val innerScriptTail = """
            if [ "${'$'}movo_mode" = command ]; then
              if [ -x "${'$'}movo_rootfs/usr/bin/env" ]; then
                exec "${'$'}movo_busybox" chroot "${'$'}movo_rootfs" /usr/bin/env -i \
                  HOME=/root USER=root LOGNAME=root SHELL=/bin/sh TERM=$termType NO_COLOR=1 \
                  LANG=C.UTF-8 LC_ALL=C.UTF-8 \
                  PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
                  /bin/sh -lc "${'$'}movo_payload"
              fi
              exec "${'$'}movo_busybox" chroot "${'$'}movo_rootfs" /bin/busybox env -i \
                HOME=/root USER=root LOGNAME=root SHELL=/bin/sh TERM=$termType NO_COLOR=1 \
                LANG=C.UTF-8 LC_ALL=C.UTF-8 \
                PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
                /bin/sh -lc "${'$'}movo_payload"
        ...
        return "$discovery; " +
            "[ -n \"${'$'}movo_busybox\" ] || { echo 'MOVO_LINUX_BUSYBOX_MISSING' >&2; exit 127; }; " +
            "movo_rootfs=$rootfs; " +
            "[ -f \"${'$'}movo_rootfs/${LinuxEnvironmentPaths.READY_MARKER}\" ] && " +
            "{ [ -x \"${'$'}movo_rootfs/bin/sh\" ] || [ -h \"${'$'}movo_rootfs/bin/sh\" ]; } && " +
            "( [ -x \"${'$'}movo_rootfs/usr/bin/env\" ] || [ -x \"${'$'}movo_rootfs/bin/busybox\" ] ) || " +
            "{ echo 'MOVO_LINUX_ENVIRONMENT_NOT_READY' >&2; exit 127; }; " +
            "\"${'$'}movo_busybox\" unshare -m --propagation private " +
            "\"${'$'}movo_busybox\" sh -c ${shellQuote(innerScript)} movo-linux " +
            "\"${'$'}movo_rootfs\" \"${'$'}movo_busybox\" $mode $payload"
    }
...
internal const val MOVO_PROCESS_OWNER_ENV = "MOVO_PROCESS_OWNER"
...
internal fun ptySupported(processSupervisor: ShellProcessSupervisor): Boolean {
    val command = AndroidBusyBox.discoveryScript() +
        "; [ -n \"\$movo_busybox\" ] || exit 1; \"\$movo_busybox\" --list 2>/dev/null | grep -qx script"
