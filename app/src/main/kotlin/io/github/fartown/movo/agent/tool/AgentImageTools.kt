...
internal class AgentImageTools(
    ...
    fun readImage(args: JSONObject): AgentModelClient.ToolResult {
        ...
        val temporaryFile = runCatching {
            File.createTempFile("movo-read-image-", ".img", imageCacheDirectory())
        }.getOrElse {
        ...
        return try {
            ...
            if (!staged) {
                if (!rootAvailable()) {
                    return sensitive(error("IMAGE_ACCESS_DENIED", "Movo 无法读取此图片；请先通过文件选择器导入或授予读取权限"))
                }
    ...
    private fun copyFailure(result: BoundedRootCommandExecutor.Result): String = when (result.exitCode) {
        ...
        23 -> error("IMAGE_STAGE_FAILED", "Root 无法将图片复制到 Movo 临时缓存")
