...
import io.github.fartown.movo.data.db.ProviderModelEntity
...
/** Movo 用户数据备份；旧备份没有角色字段时保留设备上的角色库。 */
@Serializable
internal data class MovoBackupDocument(
    val format: String = FORMAT,
    val schemaVersion: Int = SCHEMA_VERSION,
    val exportedAt: Long,
    val providers: List<MovoBackupProvider> = emptyList(),
...
internal data class MovoBackupSummary(
...
internal class MovoBackupException(message: String, cause: Throwable? = null) :
    IllegalArgumentException(message, cause)

internal object MovoBackupRepository {
    ...
    suspend fun export(context: Context, output: OutputStream): MovoBackupSummary =
        withContext(Dispatchers.IO) {
            ...
            if (bytes.size > MAX_BACKUP_BYTES) {
                throw MovoBackupException("备份文件超过 64 MiB 限制")
            }
            output.write(bytes)
            output.flush()
            document.summary()
    ...
    suspend fun import(context: Context, input: InputStream): MovoBackupSummary =
        withContext(Dispatchers.IO) {
            val document = readDocument(input)
            validate(document)
            val appContext = context.applicationContext
            val database = MovoDatabase.get(appContext)
    ...
    suspend fun inspect(input: InputStream): MovoBackupSummary = withContext(Dispatchers.IO) {
        val document = readDocument(input)
        validate(document)
        document.summary()
    }

    private suspend fun snapshot(context: Context): MovoBackupDocument {
        val appContext = context.applicationContext
        val database = MovoDatabase.get(appContext)
    ...
    private fun readDocument(input: InputStream): MovoBackupDocument {
        val bytes = input.readBytesLimited(MAX_BACKUP_BYTES)
        if (bytes.isEmpty()) throw MovoBackupException("备份文件为空")
        return runCatching {
            json.decodeFromString<MovoBackupDocument>(bytes.toString(Charsets.UTF_8))
        }.getOrElse { failure ->
            throw MovoBackupException("备份文件格式无效", failure)
        }
    }

    private fun validate(document: MovoBackupDocument) {
        if (document.format != MovoBackupDocument.FORMAT) {
            throw MovoBackupException("这不是 Movo 备份文件")
        }
        if (document.schemaVersion !in 1..MovoBackupDocument.SCHEMA_VERSION) {
            throw MovoBackupException("不支持的 Movo 备份版本：${document.schemaVersion}")
        }
        ...
        if (providerIds.size != providerIds.toSet().size || providerIds.any(String::isBlank)) {
            throw MovoBackupException("备份中的模型提供商存在重复或无效 ID")
        }
        ...
        if (document.selectedModelId != null && selectedModel == null) {
            throw MovoBackupException("备份中的当前模型不存在")
        }
        if (selectedModel != null && selectedModel.second != document.selectedProviderId) {
            throw MovoBackupException("备份中的当前模型与提供商不匹配")
        }
    ...
    private fun MovoBackupDocument.summary(): MovoBackupSummary = MovoBackupSummary(
