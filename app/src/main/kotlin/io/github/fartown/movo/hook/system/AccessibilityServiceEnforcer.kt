...
import io.github.fartown.movo.agent.accessibility.AccessibilityProtectionProtocol
import io.github.fartown.movo.core.ModuleLogger
import io.github.fartown.movo.core.safeLogType
import java.util.concurrent.atomic.AtomicBoolean

/**
 * 在 system_server 存活期间按用户选择保持 Movo 无障碍服务可用。
 *
 * 保护默认关闭；开启后只维护 owner 用户中的 Movo 组件与总开关，始终保留其他服务。
...
internal class AccessibilityServiceEnforcer(
    ...
    private fun ensureActiveObserversRegistered(context: Context) {
        ...
        if (!packageReceiverRegistered) {
            ...
            try {
                ...
                logFailure("无法监听 Movo 包变化", failure)
            }
        }
    ...
    private fun unregisterActiveObservers(context: Context) {
        ...
        if (packageReceiverRegistered) {
            try {
                ...
                logFailure("无法注销 Movo 包监听", failure)
            }
        }
        if (lifecycleReceiverRegistered) {
            try {
    ...
    private fun completeServiceRepair(
        ...
        try {
            ...
            logFailure("无法重新启用 Movo 无障碍服务", failure)
        } finally {
            repairInProgress.set(false)
        }
        if (isEnforcementEnabled(context)) {
    ...
    private fun isExpectedServiceValid(context: Context): Boolean {
        ...
        val serviceInfo = try {
            ...
            logFailure("无法校验 Movo 无障碍服务组件", failure)
            return false
        }
    ...
    private fun logRestore(
        ...
        logger.info(
            "已恢复 Movo 无障碍: reason=$reason " +
                "serviceList=$restoredServices masterSwitch=$restoredMasterSwitch",
        )
    }

    private companion object {
        const val APP_PACKAGE = "io.github.fartown.movo"
        const val SERVICE_CLASS =
            "io.github.fartown.movo.agent.accessibility.AgentAccessibilityService"
