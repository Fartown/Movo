package io.github.fartown.movo.hook.xiaoai
...
internal class XiaoAiStreamRenderer(
    ...
    fun onEvent(event: AgentEvent) {
        when (event) {
            ...
            is AgentEvent.ProviderResponseStarted -> render(
                text(R.string.injected_reasoning, "Movo is reasoning…"),
            )
            ...
            is AgentEvent.ToolStarted -> {
                if (synchronized(streamedText) { streamedText.isEmpty() }) {
                    render(text(R.string.injected_executing, "Movo is working…"))
                }
            }
    ...
    fun fail() {
        ...
        render(
            text(
                ...
                "Movo could not complete the task. Try again later",
            ),
            immediate = true,
        )
    }
    ...
    private fun text(resourceId: Int, englishFallback: String): String =
        MovoInjectedStrings.get(context, resourceId, englishFallback)
    ...
    private fun speak(text: String) {
        try {
            ...
            logger.warnThrottled("xiaoai_tts_failed") {
                "超级小爱 Movo 结果朗读失败: type=${exception.safeLogType()}"
            }
        }
    }
