...
internal class EtaWakeTileService : TileService() {
    ...
    override fun onStartListening() {
        ...
        rendering = scope.launch {
            combine(VoiceSettingsRepository.wakeSettingsFlow(), EtaWakeWordService.listeningState) { settings, state ->
                WakeTileState.of(settings.wakeEnabled, EtaWakeWordController.hasMicPermission(this@EtaWakeTileService), state)
            }.collect(::render)
        }
    }
    ...
    override fun onClick() {
        ...
        scope.launch {
            when {
                !EtaWakeWordController.hasMicPermission(this@EtaWakeTileService) -> open(
                    Intent(this@EtaWakeTileService, MainActivity::class.java).setAction(ACTION_QS_TILE_PREFERENCES),
                )
                // EtaWakeWordController observes the setting and stops the running service.
                VoiceSettingsRepository.wakeSettings().wakeEnabled -> VoiceSettingsRepository.setWakeEnabled(false)
                else -> open(Intent(this@EtaWakeTileService, WakeTileBridgeActivity::class.java))
            }
        }
    }
 ...
 * only start while an Eta activity is visible, so this stays resumed until the service has
...
internal class WakeTileBridgeActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            withResumed { EtaWakeWordService.start(this@WakeTileBridgeActivity) }
            withTimeoutOrNull(3_000) {
                EtaWakeWordService.listeningState.first {
