...
internal class VoiceConversationController(
    ...
    override fun runtimeEvent(turn: Long, event: io.github.mangi.eta.agent.runtime.AgentEvent) {
        when (event) {
            ...
            is io.github.mangi.eta.agent.runtime.AgentEvent.ToolFinished ->
                observer?.invoke("tool.finished", turn, event.name, event.success.toString())
            is io.github.mangi.eta.agent.runtime.AgentEvent.AssistantBlockDelta ->
                if (event.kind == io.github.mangi.eta.agent.runtime.AgentEvent.AssistantBlockKind.TEXT)
                    observer?.invoke("text.delta", turn, event.delta, "${event.round}:${event.index}")
            is io.github.mangi.eta.agent.runtime.AgentEvent.AssistantBlockEnd ->
                if (event.kind == io.github.mangi.eta.agent.runtime.AgentEvent.AssistantBlockKind.TEXT)
