        ...
        try {
            listOf("user", "assistant", "system").forEach { role ->
                ...
                val source = JSONArray().put(context.personaMessage())
                    .put(AgentConversationCodec.userTextMessage("第一句").put("_movo_message_id", "first"))
                ...
                assertFalse(body.toString().contains("_movo_message_id"))
    ...
    private fun assertIsolatedRequests(purpose: ProviderRequestPurpose) {
        listOf(OpenAiChatCompletionsProvider, OpenAiResponsesProvider, AnthropicMessagesProvider).forEach { provider ->
            ...
            try {
                val config = AgentModelClient.ModelConfig(
                    baseUrl = "http://127.0.0.1:${server.address.port}", apiKey = "fixture",
                    model = "fixture", openAiEndpointMode = if (provider === OpenAiResponsesProvider) io.github.fartown.movo.data.model.OpenAiEndpointMode.RESPONSES
                        else io.github.fartown.movo.data.model.OpenAiEndpointMode.CHAT_COMPLETIONS, systemPrompt = "总结规则", hostedWebSearchEnabled = true,
