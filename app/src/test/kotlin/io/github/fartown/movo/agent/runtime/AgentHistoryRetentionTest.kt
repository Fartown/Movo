...
class AgentHistoryRetentionTest {
    ...
    private val config = AgentModelClient.ModelConfig(
        ...
        reasoningEffort = io.github.fartown.movo.data.model.ReasoningEffort.OFF,
    )
    ...
    @Test fun missingChunkFailsInsteadOfReturningEmptyHistory() {
        ...
        MovoDatabase.get(context).openHelper.writableDatabase.execSQL("DELETE FROM agent_text_chunks WHERE chunk_index = 1")
    ...
    @Test fun largeRequestsAndResultsUseDescriptorsAndPreserveSummaryMetadata() {
        ...
        assertTrue(context.cacheDir.listFiles().orEmpty().none { it.name.startsWith("movo-context-") })
    }
    ...
    @Test fun localMessengerOwnsIndependentDescriptorUntilReceiverConsumesIt() {
        ...
        file.writeText(text)
        try {
            bundle.putParcelable("history_movo_text_fd", ParcelFileDescriptor.open(file, ParcelFileDescriptor.MODE_READ_ONLY))
            bundle.putLong("history_movo_text_bytes", file.length())
    ...
    @Test fun incorrectDescriptorLengthIsRejectedAndClosed() {
        ...
        bundle.putLong("history_movo_text_bytes", 1)
        assertThrows(IllegalArgumentException::class.java) { AgentWireText.read(bundle, "history") }
        assertFalse(bundle.containsKey("history_movo_text_fd"))
    }
}
