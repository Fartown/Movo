...
class CharacterRepositoryTest {
    ...
    fun setUp() {
        ...
        context.getSharedPreferences("movo_roleplay", 0).edit().clear().commit()
    ...
    fun largeCardsPersonaAndConversationBindingsUseChunkedStorage() = runBlocking {
        ...
        MovoDatabase.get(context).openHelper.readableDatabase.query(
            "SELECT card_json FROM roleplay_characters WHERE id = ?", arrayOf(profile.id),
        ).use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertTrue(cursor.getString(0).startsWith("@movo:chunks:v1:"))
        }
