...
import io.github.fartown.movo.agent.roleplay.RoleplayReplyRevision
...
@Config(sdk = [36])
class MovoBackupRepositoryTest {
    ...
    fun exportAndImportRestoresProvidersConversationsAndMemory() = runBlocking {
        ...
        val imported = MovoBackupRepository.import(
            context,
    ...
    fun roleplayBackupRestoresCardsPersonaAvatarMemoryAndLargeSessionMetadata() = runBlocking {
        ...
        MovoDatabase.get(context).conversationDao().insertConversations(listOf(
        ...
        MovoDatabase.get(context).conversationDao().insertMessages(listOf(
            ConversationMessageEntity("role-message", "role-session", 0, "assistant", "版本".repeat(40_000)),
        ))
        val output = ByteArrayOutputStream()
        assertEquals(1, MovoBackupRepository.export(context, output).characterCount)
        ...
        MovoBackupRepository.import(context, ByteArrayInputStream(output.toByteArray()))
        ...
        val row = MovoDatabase.get(context).conversationDao().conversationEntities().single()
    ...
    fun versionOneBackupRemainsReadable() = runBlocking {
        val summary = MovoBackupRepository.inspect(ByteArrayInputStream(
            """{"format":"movo-backup","schemaVersion":1,"exportedAt":0}""".toByteArray(),
        ))
    ...
    fun memoryWriteFailureRollsBackDatabaseAndEarlierMemoryFiles() = runBlocking {
        ...
        val database = MovoDatabase.get(context)
        ...
        val document = MovoBackupDocument(exportedAt = 0, memoryMd = "新的现实记忆",
        ...
        try {
            var failed = false
            try {
                MovoBackupRepository.import(context, ByteArrayInputStream(Json.encodeToString(document).toByteArray()))
    ...
    fun invalidRevisionBackupIsRejectedBeforeExistingConversationsAreChanged() = runBlocking {
        ...
        invalidStates.forEach { revisions ->
            val document = MovoBackupDocument(exportedAt = 0,
