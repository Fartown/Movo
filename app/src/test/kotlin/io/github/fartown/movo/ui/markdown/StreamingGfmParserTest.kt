...
class StreamingGfmParserTest {
    ...
    fun completedSnapshotIncludesReferenceLinksBeforeStableHandoff() {
        val content = "查看 [文档][movo]\n\n[movo]: https://example.com/docs"
        val snapshot = StreamingGfmParserSession().parse(content, isComplete = true)

        assertTrue(snapshot.state.linksLookedUp)
        assertEquals("https://example.com/docs", snapshot.state.referenceLinkHandler.find("[movo]"))
    }

    @Test
    fun correctedSnapshotDoesNotRetainOrMutatePreviousLinkDefinitions() {
        val parser = StreamingGfmParserSession()
        val original = parser.parse("[文档][movo]\n\n[movo]: https://example.com/docs", isComplete = true)
        val replacement = parser.parse("[文档][movo]", isComplete = true)

        assertEquals("", replacement.state.referenceLinkHandler.find("[movo]"))
        assertEquals("https://example.com/docs", original.state.referenceLinkHandler.find("[movo]"))
    }
