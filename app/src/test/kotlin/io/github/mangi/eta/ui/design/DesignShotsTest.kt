package io.github.mangi.eta.ui.design
...
import io.github.mangi.eta.ui.app.AgentAppShell
import io.github.mangi.eta.ui.components.AgentChatInputBar
import io.github.mangi.eta.ui.components.AgentWorkProcess
import io.github.mangi.eta.ui.components.ChatMessageItem
import io.github.mangi.eta.ui.components.MovoHomeContent
...
class DesignShotsTest {
    ...
    private fun steps(running: Boolean = true): Array<io.github.mangi.eta.ui.model.AgentChatMessageUi> {
    ...
    fun runDetail() = shot("21-run-detail") {
        io.github.mangi.eta.ui.screens.run.RunDetailScreen(
    ...
    private fun overlayState(phase: io.github.mangi.eta.agent.overlay.AgentOverlayPhase): io.github.mangi.eta.agent.overlay.AgentOverlayState {
        val now = System.currentTimeMillis()
        return io.github.mangi.eta.agent.overlay.AgentOverlayState(
            phase = phase,
            status = io.github.mangi.eta.agent.overlay.AgentOverlayStatus.RunningTool("tap"),
            steps = listOf(
                io.github.mangi.eta.agent.overlay.OverlayStep("1", "打开「美团」", io.github.mangi.eta.agent.overlay.OverlayStepStatus.DONE),
                io.github.mangi.eta.agent.overlay.OverlayStep("2", "搜索「瑞幸 生椰拿铁」", io.github.mangi.eta.agent.overlay.OverlayStepStatus.DONE),
                io.github.mangi.eta.agent.overlay.OverlayStep("3", "点击「少冰」", io.github.mangi.eta.agent.overlay.OverlayStepStatus.RUNNING),
                io.github.mangi.eta.agent.overlay.OverlayStep("4", "选择规格", io.github.mangi.eta.agent.overlay.OverlayStepStatus.RUNNING),
            ),
            startedAtMillis = now - 18_000,
            pausedAtMillis = if (phase == io.github.mangi.eta.agent.overlay.AgentOverlayPhase.PAUSED) now else null,
        )
    }
    ...
    fun overlay() = shot("07-overlay") {
        Column(
            ...
            androidx.compose.foundation.layout.Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                io.github.mangi.eta.agent.overlay.OrbMode.entries.forEach { mode ->
                    io.github.mangi.eta.agent.overlay.AgentOverlayOrb(mode = mode, onTap = {})
                }
            }
            androidx.compose.foundation.layout.Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                // 移除区 + 吸附时的悬浮球（放大 1.1）
                io.github.mangi.eta.agent.overlay.AgentOverlayRemoveZone(visible = true)
                io.github.mangi.eta.agent.overlay.AgentOverlayOrb(mode = io.github.mangi.eta.agent.overlay.OrbMode.STANDBY, onTap = {}, engaged = true)
            }
            io.github.mangi.eta.agent.overlay.AgentOverlayBubble(
                state = overlayState(io.github.mangi.eta.agent.overlay.AgentOverlayPhase.RUNNING),
                onCollapse = {}, onPause = {}, onResume = {}, onStop = {}, onSupplementModeChange = {}, onSupplement = {},
            )
            io.github.mangi.eta.agent.overlay.AgentOverlayBubble(
                state = overlayState(io.github.mangi.eta.agent.overlay.AgentOverlayPhase.PAUSED),
                onCollapse = {}, onPause = {}, onResume = {}, onStop = {}, onSupplementModeChange = {}, onSupplement = {},
            )
            ...
            io.github.mangi.eta.agent.overlay.AgentOverlayBubble(
                state = overlayState(io.github.mangi.eta.agent.overlay.AgentOverlayPhase.RUNNING),
                onCollapse = {}, onPause = {}, onResume = {}, onStop = {}, onSupplementModeChange = {}, onSupplement = {},
                voice = io.github.mangi.eta.agent.voice.session.VoiceSessionUiState(
                    channel = io.github.mangi.eta.agent.voice.session.VoiceChannel.Hearing,
