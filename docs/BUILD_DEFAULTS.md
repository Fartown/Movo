...
在项目根目录复制 `.env.example` 为 `.env`，填写 `MOVO_DEFAULT_API_KEY`，按需编辑服务地址、模型 ID 和名称。Gradle 构建 debug/release 时会把这些值编入 APK；同名环境变量优先于 `.env`，未提供完整的 URL、Key、模型 ID 时沿用原有提供商初始化。
