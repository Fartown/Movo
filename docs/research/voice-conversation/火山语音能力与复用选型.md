...
具体复用落点：`MovoAsrEngine.kt` 扩分句事件；`DoubaoBidirectionalAsrEngine.kt` 透传；`DoubaoSaucProtocol.kt` 保留解析；现有 `MovoWakeWordService`/麦协调器承载会话；`AgentRuntimeClient` 保留模型与执行；TTS 新增一个 SDK 适配层。原主方案拟新增的网络合成器和基础播放器先不各自实现一套。
